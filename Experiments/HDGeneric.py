...
class NeuralHD:
    def __init__(self, classes : int, features : int, dim : int = 400, batch_size=1,trainopt=3,bestinclass=False,mu=0,sigma=1, learningrate=.003):
        ...
        self.base = torch.empty(self.dimension).uniform_(0.0, 2*math.pi)
        ...
        self.basis = torch.normal(0,1,size=(self.dimension,self.nFeatures))
        # Initialize classification hypervectors
        self.classes = torch.zeros((self.nClasses, self.dimension))
        self.prevacc=0
        self.trainoption=trainopt
        self.trainfunctions=[self.train,self.train2,self.train3]
        # self.learningrate=.1
        # self.hdc = HD_classifier(self.dimension, self.nClasses, 0)
    ...
    def train2(self,h,y):
        ...
        assert self.dimension == h.size(1)
        #if self.first_fit:
    ...
    def fit(self,traindata, trainlabels,
        ...
        amountDrop = int(fractionToDrop * self.dimension)#self.param.D?
